...
public class StarbucksWebAppWindTunnel {
	...
	public static void main(String[] args) throws MalformedURLException, IOException {
		...
		capabilities.setCapability("scriptName", "WindTunnel-StarbucksWebAppWindTunnel");
	...
	private static void runStarbucksTest(DesiredCapabilities capabilities) throws IOException {
		...
		try {
			...
			// Go Home
			WindTunnelUtils.pointOfInterest(driver, "Go Home & close app", WindTunnelUtils.SUCCESS); 
			...
			sleep(3000);
			// Restart Application
			WindTunnelUtils.pointOfInterest(driver, "Launch app & start app vitals", WindTunnelUtils.SUCCESS);
			...
			long launchTimer = MyUtils.getUXTimer(driver);
			WindTunnelUtils.reportTimer(driver, launchTimer, 1000, app + " Launch Time", "Launch");
			...
			long measuredNavTimer = MyUtils.getUXTimer(driver);
			// Click on a store address
			WindTunnelUtils.pointOfInterest(driver, "Get directions to store", WindTunnelUtils.SUCCESS);
			MyUtils.ocrTextClick(driver, "mi", -1, 10, 50);
			String result = MyUtils.ocrTextCheck(driver, "Directions", -1, 60);
			measuredNavTimer += MyUtils.getUXTimer(driver);
			...
			if (result.equalsIgnoreCase("true")) {
				...
				measuredNavTimer += MyUtils.getUXTimer(driver);
				// How long did it take to launch?
				WindTunnelUtils.reportTimer(driver, measuredNavTimer, 500, app + " Navigate to store", "Nav 2 store");
			}
			else {
			...
			try {
				driver.close();
				String fileName = StarbucksWebAppWindTunnel.class.getSimpleName() + "_" + System.currentTimeMillis();
